...
public class AbsoluteProjectionTraverser implements GraphTraverser<SequenceSegment> {
     ...
     * @param reference
     *              Reference used to keep track of the absolute mapping of the graph.
     */
    public AbsoluteProjectionTraverser(String reference) {
     ...
     */
    private void traverseVertexForward(Graph<SequenceSegment> graph, SequenceSegment vertex) {
        long position = vertex.getAbsStart();
        
        if (vertex.getSources().contains(reference) && vertex.getContent() instanceof SegmentString) {
     ...
     *          The vertex that is being projected.
     */
    private void traverseVertexBackward(Graph<SequenceSegment> graph, SequenceSegment vertex) {
